...
<header class="bg-white py-4 shadow-md sticky top-0 z-10">
    <div class="container mx-auto px-4 flex justify-between items-center">
        <h1 class="text-2xl font-['Segoe_Print'] font-bold">Craftlab</h1>
        <a href="/" class="bg-gray-700 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded">Home</a>
    </div>
</header>
...
<form class="container mx-auto p-5" method="POST" enctype="multipart/form-data">
    ...
    <div class="canvas-container mb-3">
        ...
        {#if files && files.length}
            <div class="mt-2">
                <button type="button" on:click={() => applyFilter('grayscale')} class="bg-gray-700 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded">Grayscale</button>
                <button type="button" on:click={() => applyFilter('sepia')} class="bg-gray-700 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded">Sepia</button>
            </div>
        {:else}
    ...
    <button type="submit" class="mt-4 text-white bg-gray-700 hover:bg-gray-900 font-medium rounded-lg text-sm px-5 py-2.5">Post</button>
</form>
